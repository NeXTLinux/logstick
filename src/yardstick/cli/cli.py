...
from logstick import store
from logstick.cli import config, label, result


@click.option("--verbose", "-v", default=False, help="show logs", is_flag=True)
@click.option("--config", "-c", "config_path", default=".logstick.yaml", help="override config path")
...
@cli.command(name="version", help="show the installed version of logstick")
@click.pass_obj
def version(_: config.Application):
    d = pkg_resources.get_distribution("logstick")
    if not d:
        raise RuntimeError("logstick install information could not be found")
    print(repr(d))

...
function run() {
    tmp_file=$(mktemp /tmp/logstick-test.XXXXXX)
    temp_files+=( $tmp_file )
...
run logstick result clear

run logstick result capture -r test

run logstick result list -r test
...
assert_last_output_contains "docker.io/nextlinux/test_images:java-56d52bc@sha256:10008791acbc5866de04108746a02a0c4029ce3a4400a9b3dad45d7f2245f9da"

run logstick result compare $(logstick result list -r test --ids -t grype)
...
run logstick label apply $(logstick result list -r test --ids -t grype@v0.50.2)
